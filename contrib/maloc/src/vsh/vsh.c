    ...
    if ( !stat(Vsh_getenv(thee,"ENV"), &fInfo) ) {
        if (VS_ISREG(fInfo.st_mode)) {
            ...
            if (thee->scUnit != VNULL) {
                ...
                strncpy(thee->cinName,Vsh_getenv(thee,"ENV"),80);
...
VPUBLIC int Vsh_builtIn(Vsh *thee, int argc, char **argv)
{
    ...
    switch (theCmd) {
      ...
      case vshcom_dot:
        if (argc <= 1) {
            ...
            if (VS_ISREG(fInfo.st_mode)) {
                ...
                if (thee->scUnit != VNULL) {
                    thee->cinUnit = thee->scUnit;
                    strncpy(thee->cinName,argv[1],80);
                    Vnm_print(0,"Starting <%s> script named <%s>\n",
                        thee->PR,thee->cinName);
                } else {
      ...
      case vshcom_sockg:
        ...
        strncpy(sofmt,Vsh_getenv(thee,"OSFMT"),80);
        if (!strcmp("unix",Vsh_getenv(thee,"OSKEY"))) {
            strncpy(sokey,"Mcs",80);
        ...
        if (!strcmp("1x1",Vsh_getenv(thee,"GVLO"))) {
            sprintf(sysc,"geomview -nopanels -wpos 436,445@0,520 -%s 0",sokey);
            Vnm_systemBack(sysc);
            ...
            sprintf(sysc,"geomview -nopanels -wpos 436,445@0,520 -%s 0",sokey);
            Vnm_systemBack(sysc);
            sprintf(sysc,"geomview -nopanels -wpos 436,445@0,50  -%s 1",sokey);
            Vnm_systemBack(sysc);
            if (!strcmp("inet",Vsh_getenv(thee,"OSKEY"))) {
                Vnm_systemBack("mcbridge -i2u 0 0");
            ...
            sprintf(sysc,"geomview -nopanels -wpos 436,287@0,684 -%s 0",sokey);
            Vnm_systemBack(sysc);
            sprintf(sysc,"geomview -nopanels -wpos 436,287@0,367 -%s 1",sokey);
            Vnm_systemBack(sysc);
            sprintf(sysc,"geomview -nopanels -wpos 436,287@0,50  -%s 2",sokey);
            Vnm_systemBack(sysc);
            if (!strcmp("inet",Vsh_getenv(thee,"OSKEY"))) {
                Vnm_systemBack("mcbridge -i2u 0 0");
                Vnm_systemBack("mcbridge -i2u 1 1");
      ...
      case vshcom_sockm:
        ...
        if (!strcmp("unix",Vsh_getenv(thee,"OSKEY"))) {
            ...
            strncpy(sokey,"Mci",80);
        } else {
            strncpy(sokey,"Mci",80);
        }
        if (!strcmp("1x1",Vsh_getenv(thee,"GVLO"))) {
            sprintf(sysc,"mcsg -%s -wpos 436,445@0,520 -%s 0",sofmt,sokey);
            Vnm_systemBack(sysc);
        } else if (!strcmp("2x1",Vsh_getenv(thee,"GVLO"))) {
            sprintf(sysc,"mcsg -%s -wpos 436,445@0,520 -%s 0",sofmt,sokey);
            Vnm_systemBack(sysc);
            sprintf(sysc,"mcsg -%s -wpos 436,445@0,50  -%s 1",sofmt,sokey);
            Vnm_systemBack(sysc);
        } else if (!strcmp("3x1",Vsh_getenv(thee,"GVLO"))) {
            sprintf(sysc,"mcsg -%s -wpos 436,287@0,684 -%s 0",sofmt,sokey);
            Vnm_systemBack(sysc);
            sprintf(sysc,"mcsg -%s -wpos 436,287@0,367 -%s 1",sofmt,sokey);
            Vnm_systemBack(sysc);
            sprintf(sysc,"mcsg -%s -wpos 436,287@0,50  -%s 2",sofmt,sokey);
            Vnm_systemBack(sysc);
        } else if (!strcmp("4x1",Vsh_getenv(thee,"GVLO"))) {
            ...
            sprintf(sysc,"mcsg -%s -wpos 425,345@0,0 -%s 0",sofmt,sokey);
            Vnm_systemBack(sysc);
            sprintf(sysc,"mcsg -%s -wpos 425,345@0,369 -%s 1",sofmt,sokey);
            Vnm_systemBack(sysc);
        } else {
            Vnm_print(2,"%s: %s: Incorrect argument <%s>\n",
                thee->PR,"GVLO",Vsh_getenv(thee,"GVLO"));
