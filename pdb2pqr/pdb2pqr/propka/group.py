...
class Group:
    def __init__(self, atom):
        ...
        if self.atom.type=='atom':
            self.label = '%-3s%4d%2s'%(self.residue_type, atom.resNumb, atom.chain_id)
...
#            self.label = '%1s%1s%1s%4d%2s'%(self.residue_type[1], atom.element,atom.name[-1], atom.resNumb, atom.chain_id)
        else:
