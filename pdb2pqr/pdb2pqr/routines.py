...
class Routines:
    ...
    def addHydrogens(self):
        ...
        This requires either the rebuildTetrahedral function for tetrahedral
    ...
    def debumpResidue(self, residue, conflictnames):
        ...
        """
        # Initialize some variables
        # TODO - these variables should probably be at the top of the file.
        ANGLE_STEPS = 72
        ...
        for _ in range(ANGLE_TEST_COUNT):
            ...
            if foundImprovement:
                ...
                _LOGGER.debug(err.format(best=repr(bestscore), angle=repr(bestangle)))
                _LOGGER.debug("New conflict set: %s", str(currentConflictNames))
            else:
    ...
    def runPROPKA31(self, pka_options):
        ...
        # .pdb location)
        pka_molecule = propka.molecular_container.Molecular_container(HFreeProteinFile.name,
                                                                      pka_options)
    ...
    def apply_pka_values(self, ff, ph, pkadic):
        ...
        if len(warnings) > 0:
            init = ("PDB2PKA determined the following residues to be in a protonation "
                    "state not supported by the {ff} forcefield.  All were reset to "
                    "their pH 7.0 model titration state.")
