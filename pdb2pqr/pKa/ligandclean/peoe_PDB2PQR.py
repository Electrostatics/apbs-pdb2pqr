...
#        'N.4':  (11.54-8.0,  10.28,  1.36,  28.00),  ### for lysine - PC 22.01.05
...
# Carbon
        'C.3':  ( 7.98,       9.18,  1.88,  19.04),
        'C.cat':( 7.98,       9.18,  1.88,  19.04),
        'C.2':  ( 8.79,       9.32,  1.51,  19.62),
        'C.ar': ( 7.98,       9.18,  1.88,  19.04),
        'C.1':  (10.39,       9.45,  0.73,  20.57),
# Nitrogen
        'N.3':  (11.54,      10.28,  1.36,  23.72),
	'N.4':  (11.54,      10.28,  1.36,  23.72), #
        'N.ar': (12.87,      11.15,  0.85,  24.87),
        'N.2':  (12.87,      11.15,  0.85,  24.87),
        'N.pl3':(12.87,      11.15,  0.85,  24.87),
        'N.am': (12.87,      11.15,  0.85,  24.87),
        'N.1':  (15.68,      11.70, -0.27,  27.11),
# Oxygen	
        'O.3'  :  (14.18,      12.92,  1.39,  28.49),
        'O.2'  :  (17.07,      13.79,  0.47,  31.33),
        'O.co2':  (15.25,      13.79,  0.47,  31.33),
	'O.OH' :  (14.18,      12.92,  1.39,  28.49),
# Halogenides
        'F' :   (12.36,      13.85,  2.31,  30.82),
        'Cl':   ( 9.38,       9.69,  1.35,  22.04),
        'Br':   (10.08,       8.47,  1.16,  19.71),
        'I' :   ( 9.90,       7.96,  0.96,  18.82),
# etc
        'S.3':  (10.13,       9.13,  1.38,  20.65),
	'S.2':  (10.13,       9.13,  1.38,  20.65), #
	'P.3':  (10.13,       9.13,  1.38,  20.65), #
        }



#Chargeterms = Chargeterms_ref


def PEOE( atoms, damp=0.778, k=1.6):
        ...
        counter = 0
	#
        # setting all charges to 0.0
#	for a in atoms.atoms:
#		a.charge = 0.0
	#
	for a in atoms.atoms: # lAtoms
		...
		if a.sybylType == 'O.3': # and 'H' in map(lambda x: getattr(x,'sybylType'), a.lBondedAtoms):    # if it is an alcohole, use special type
			#print "----------alcohol!", a.name, a.sybylType
        ...
        if abs_qges != 0.0:
          ...
          for b in range(1,cycles):
            for i in atoms.atoms: # lAtoms
		      ...
		      #dchi  = (calcchi(j, j.charge) - i.chi)
		      #if dchi > 0.0: i.dq += (dchi / calcchi(i, +1) * (damp**b))
		      #else:          i.dq += (dchi / calcchi(j, +1) * (damp**b))
            for i in atoms.atoms: # lAtoms
          ...
          for a in range(1,7):         
			...
			for j in i.intrabonds: ### lBondedAtoms
				...
				#dchi  = (calcchi(j, j.charge) - i.chi)
                                #if dchi > 0.0: i.dq += (dchi / calcchi(i, +1) * (damp**a)) 
                                #else:          i.dq += (dchi / calcchi(j, +1) * (damp**a))
