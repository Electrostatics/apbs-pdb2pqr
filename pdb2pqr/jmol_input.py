...
serviceURL='http://ws.nbcr.net/opal/services/ApbsOpalService'
cgitb.enable()
...
def initVars():
    ...
    if not form.has_key("jobid"):
        ...
        aoFile = open('./tmp/%s/%s-ao' % (jobid, jobid))
        ...
        if APBS_OPAL_URL!="":
            ...
            resp = appLocator.getAppServicePort(serviceURL).getOutputs(getOutputsRequest(apbsOpalJobID))
