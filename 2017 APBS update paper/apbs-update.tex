...
This manuscript provides an overview of the new capabilities in the APBS software and its associated tools since their release in 2001 \cite{Holst2000, Baker2000, Baker2001b, Baker2001}; more detailed documentation for these tools is available on the APBS website \cite{APBSweb}.
...
Command-line versions of APBS can be run locally or through web services provided by the NBCR-devloped Opal toolkit \cite{Opal}.
...
Finally, APBS can run through other molecular simulation programs such ash AMBER \cite{AMBER}, CHARMM \cite{CHARMM}, NAMD \cite{NAMD}, and TINKER \cite{TINKER}.
...
\end{figure}
%%%
This workflow is supported by the APBS tool suite with specific support by PDB2PQR for preparing biomolecular structures (see Section~\ref{sec:pdb2pqr}) and support by APBS for performing electrostatics calculations (see Section~\ref{sec:apbs}).
...
PDB2PQR currently supports charge/radii force fields from AMBER99 \cite{AMBER99}, CHARMM22 (\texttt{par\_all27\_prot\_na.prm}) \cite{CHARMM22}, PARSE \cite{Sitkoff1994}, PEOEPB \cite{Czodrowski2006}, Swanson et al.~\cite{Swanson2005a}, and Tan et al.~\cite{Tan2006}.
...
The finite difference PMG solver \cite{PMGweb, PMG} trades speed and efficiency for the high-accuracy and high-detail solutions of the finite element FEtk library.
...
This ``Clean OO C'' \cite{Holst2001} has enabled the long-term sustainability of APBS and by combining an object-oriented design framework with the portability and speed of C.
...
\section{General keywords for implicit solvent calculations}
...
\begin{itemize}
	...
	\item \keyword{pdie} and \keyword{sdie}:  These keywords specify the dielectric coefficient values for the biomolecular interior (\keyword{pdie}) and bulk solvent (\keyword{sdie}).
